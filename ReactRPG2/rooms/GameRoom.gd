...
func get_exit_description() -> String:
	return " Exits: " + Types.wrap_item_text("  ".join(PackedStringArray(exits.keys()))) #return "Exits: " + " ".join(keys(PackedStringArray(Types.wrap_location_text(exits))))
...
func connect_exit_unlocked(direction: String, room, room_2_override_name = "null"):
	return _connect_exit(direction, room, false, room_2_override_name) # false not really required since its default
